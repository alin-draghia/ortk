...
void convert_pascal_v1(const std::vector<std::string>& filenames, dlib::image_dataset_metadata::dataset& dataset)
{
	...
	for (unsigned long i = 0; i < filenames.size(); ++i)
    {
        try
        {
			parse_annotation_file(filenames[i], img, name);

            dataset.name = name;
            //img.filename = strip_path(figure_out_full_path_to_image(parser[i], img.filename), parent_dir);
			//img.filename = figure_out_full_path_to_image(filenames[i], img.filename);
			(void)img.filename;
        ...
        {
            cout << "Error while processing file " << filenames[i] << endl << endl;
            throw;
        }
    }
